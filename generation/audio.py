...
def mix_audio_tracks(
    ...
    if ambient_path is not None:
        filter_complex = (
            ...
            f"volume={settings.narration_volume},atrim=0:{total_duration}[voice];"
            f"[1:a]aformat=sample_fmts=fltp:sample_rates={settings.audio_sample_rate}:channel_layouts=stereo,"
            f"volume={AMBIENT_BASE_VOLUME},atrim=0:{total_duration},volume='{ambient_shape_expr}'[ambient_base];"
            f"[ambient_base][voice]sidechaincompress=threshold={AMBIENT_DUCK_THRESHOLD}:ratio={AMBIENT_DUCK_RATIO}:"
            f"attack={AMBIENT_DUCK_ATTACK_MS}:release={AMBIENT_DUCK_RELEASE_MS}:level_sc={AMBIENT_DUCK_LEVEL_SC}"
            f"[ambient];"
            f"[2:a]aformat=sample_fmts=fltp:sample_rates={settings.audio_sample_rate}:channel_layouts=stereo,"
            f"volume={MUSIC_DUCK_BASE_VOLUME},atrim=0:{total_duration}[music];"
            f"[music][voice]sidechaincompress=threshold={MUSIC_DUCK_THRESHOLD}:ratio={MUSIC_DUCK_RATIO}:"
            f"attack={MUSIC_DUCK_ATTACK_MS}:release={MUSIC_DUCK_RELEASE_MS}:level_sc={MUSIC_DUCK_LEVEL_SC}"
            f"[ducked_music];[ducked_music]volume='{music_shape_expr}'[ducked_music_shaped];"
            f"[voice][ambient][ducked_music_shaped]amix=inputs=3:duration=first:dropout_transition=3,"
        ...
        filter_complex = (
            ...
            f"attack={MUSIC_DUCK_ATTACK_MS}:release={MUSIC_DUCK_RELEASE_MS}:level_sc={MUSIC_DUCK_LEVEL_SC}"
            f"[ducked_music];[ducked_music]volume='{music_shape_expr}'[ducked_music_shaped];"
            f"[voice][ducked_music_shaped]amix=inputs=2:duration=first:dropout_transition=3,"
            f"afade=t=in:st=0:d={FINAL_AUDIO_FADE_IN_SECONDS},"
            f"afade=t=out:st={fade_out_start}:d={FINAL_AUDIO_FADE_OUT_SECONDS}[out]"
        )
    command = inputs + [
